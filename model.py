# Adapted from https://github.com/divelab/AIRS/blob/main/OpenProt/LatentDiff/LatentDiff/protein_autoencoder/model.py
...
class EGNNPooling(torch.nn.Module):
    ...
    def forward(self, h, coords, batch=None, batched_data=None, edge_index=None):
        ...
        h, coords = self.egnn(h, coords, data.edge_index, edge_attr, data.batch)
...
class Encoder(torch.nn.Module):
    def __init__(self, n_feat=1, hidden_dim=32, out_node_dim=32, in_edge_dim=32, layers=1,
...
class LatticeAuto(torch.nn.Module):
    def __init__(self, layers=3, mp_steps=4, num_types=2, type_dim=2, hidden_dim=32, out_node_dim=32, in_edge_dim=32,
                 output_pad_dim=1, pooling=True, up_mlp=False, residual=True, noise=False, transpose=False, attn=False,
                 stride=2, kernel=3, padding=1):
        super(LatticeAuto, self).__init__()
        ...
        self.atom_type_embedding = torch.nn.Embedding(num_types, hidden_dim)
    ...
    def forward(self, batched_data):
        ...
        x, coords, edge_index, batch = batched_data.x, batched_data.coords, batched_data.edge_index, batched_data.batch

        if self.noise:
            coords = self.add_noise(coords)

        h = self.atom_type_embedding(x.squeeze(1).long()).to(device)
